...
$cinEncadrant = $_POST["cinEncadrant"] ?? null;
$motpasseEncadrant_input = $_POST["motpasseEncadrant"] ?? null; // Renommé pour clarté
$valider = $_POST["valider"] ?? null;
$erreur = "";

if (isset($valider)) {
    if (empty($cinEncadrant) || empty($motpasseEncadrant_input)) {
        ...
        if (!$conn || mysqli_connect_errno()) {
            ...
            $stmt = mysqli_prepare($conn, "SELECT idEncadrant, nomEncadrant, prenomEncadrant, motpasseEncadrant FROM Encadrant WHERE cinEncadrant = ?");

            if ($stmt) {
                mysqli_stmt_bind_param($stmt, "s", $cinEncadrant);
                mysqli_stmt_execute($stmt);
                $result = mysqli_stmt_get_result($stmt);

                if ($user = mysqli_fetch_assoc($result)) {
                    // Vérifier le mot de passe entré contre le hash stocké dans la base de données
                    if (password_verify($motpasseEncadrant_input, $user['motpasseEncadrant'])) {
                        // Le mot de passe est correct
                        $_SESSION["prenomNom"] = ucfirst(strtolower($user["prenomEncadrant"])) . " " . strtoupper($user["nomEncadrant"]);
                        $_SESSION["idEncadrant"] = $user["idEncadrant"]; // Optionnel: stocker l'ID de l'étudiant
                        $_SESSION["autoriser"] = "oui"; // Marqueur d'autorisation
                        
                        // Redirection vers la page de session de l'étudiant
                        header("location:Encadrant.php"); // Doit être c:\xampp\htdocs\GestionPFE\Session.php
...
<head>
    <meta charset="utf-8" />
    <title>Authentification Encadrant - GestionPFE</title>
    <style>
          body { 
...
<body>
    ...
    <div class="container">
        <h1>Authentification Encadrant</h1>
        <?php if (!empty($erreur)): ?>
            <div class="erreur"><?php echo htmlspecialchars($erreur); ?></div>
        <?php endif; ?>
        <form method="post" action="">
            <div class="form-group">
                <label for="cinEncadrant">CIN Encadrant :</label>
                <input type="text" id="cinEncadrant" name="cinEncadrant" placeholder="Entrez votre CIN" value="<?php echo htmlspecialchars($cinEncadrant ?? ''); ?>" required />
            </div>
            <div class="form-group">
                <label for="motpasseEncadrant">Mot de passe :</label>
                <input type="password" id="motpasseEncadrant" name="motpasseEncadrant" placeholder="Entrez votre mot de passe" required />
            </div>
