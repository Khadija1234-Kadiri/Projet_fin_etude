...
$cinEtudiant = $_POST["cinEtudiant"] ?? null;
$motpasseEtudiant_input = $_POST["motpasseEtudiant"] ?? null; // Renommé pour clarté
$valider = $_POST["valider"] ?? null;
$erreur = "";

if (isset($valider)) {
    if (empty($cinEtudiant) || empty($motpasseEtudiant_input)) {
        ...
        if (!$conn || mysqli_connect_errno()) {
            ...
            $stmt = mysqli_prepare($conn, "SELECT idEtudiant, nomEtudiant, prenomEtudiant, motpasseEtudiant FROM Etudiant WHERE cinEtudiant = ?");

            if ($stmt) {
                mysqli_stmt_bind_param($stmt, "s", $cinEtudiant);
                mysqli_stmt_execute($stmt);
                $result = mysqli_stmt_get_result($stmt);

                if ($user = mysqli_fetch_assoc($result)) {
                    // Vérifier le mot de passe entré contre le hash stocké dans la base de données
                    if (password_verify($motpasseEtudiant_input, $user['motpasseEtudiant'])) {
                        ...
                        header("location:Etudiant.php"); // Doit être c:\xampp\htdocs\GestionPFE\Session.php
    ...
    <div class="container">
        ...
        <form method="post" action="">
            <div class="form-group">
                <label for="cinEtudiant">CIN Étudiant :</label>
                <input type="text" id="cinEtudiant" name="cinEtudiant" placeholder="Entrez votre CIN" value="<?php echo htmlspecialchars($cinEtudiant ?? ''); ?>" required />
            </div>
            <div class="form-group">
                <label for="motpasseEtudiant">Mot de passe :</label>
                <input type="password" id="motpasseEtudiant" name="motpasseEtudiant" placeholder="Entrez votre mot de passe" required />
            </div>
            <input type="submit" name="valider" value="S'authentification" />
             <p id="verification">
    <a href="verificationEtudiant.php"  >mot de passe oublie ?</a>
</p>
        </form>
    </div>
</body>
