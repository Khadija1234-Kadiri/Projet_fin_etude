...
$id_liste = 0;
if (isset($_GET['id'])) {
    $id_liste = filter_var($_GET['id'], FILTER_VALIDATE_INT);
}

if (!$id_liste || $id_liste <= 0) {
    header("HTTP/1.0 400 Bad Request");
    echo "ID de liste invalide ou manquant.";
    exit();
}
...
$sql = "SELECT Fichier_pdf, nom_fichier, type_mime, taille FROM Liste WHERE idListe = ?";
$stmt = mysqli_prepare($conn, $sql);

if ($stmt) {
    mysqli_stmt_bind_param($stmt, "i", $id_liste);
